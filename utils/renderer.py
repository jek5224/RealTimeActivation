...
class PyRenderer_video:
    ...
    def __call__(self, verts=None, img=np.zeros((224, 224, 3)), cam=np.array([1, 0, 0]),
        ...
        self.renderer.viewport_width = resolution[1]
...
class PyRenderer:
    ...
    def __call__(self, verts, faces=None, img=np.zeros((224, 224, 3)), cam=np.array([1, 0, 0]),
                 ...
                 smpl_kp_3d=None, kp_3d=None):

        # faces is None -> faces info containing vertex indices come from smpl
        if faces is None:
            faces = self.faces[mesh_type]
        # make Trimesh SMPL mesh
        mesh = trimesh.Trimesh(vertices=verts, faces=faces, process=False)

        # Rotate SMPL mesh around x axis 180 degrees
        ...
        # If rotation info is given, rotate mesh
        if angle and axis:
        ...
        if iwp_mode:
            ...
            # reads predicted camera output from network
            elif len(cam) == 3:
                sx, tx, ty = cam
                sy = sx
                camera_translation = np.array([- tx, ty, 2 * focal_length[0] / (resolution[0] * sy + 1e-9)])
        ...
        # Convert Trimesh to pyrender and add in scene
        ...
        for item in image_list:
            # scale_ratio is 1, ignore this if statement
            if scale_ratio != 1:
        ...
        # if smpl_kp_3d is not None or kp_3d is not None:
        #     if smpl_kp_3d is not None:
        #         sm = trimesh.creation.uv_sphere(radius=0.01)
        #         sm.visual.vertex_colors = [0.1,0.1,1.0]
        #         tfs = np.tile(np.eye(4), (len(smpl_kp_3d), 1, 1))
        #         pts = smpl_kp_3d.copy()
        ...
        #         smpl_kp_mesh = pyrender.Mesh.from_trimesh(sm, poses=tfs)
        #         smpl_kp_mesh_node = self.scene.add(smpl_kp_mesh, 'smpl_kp')

        #     if kp_3d is not None:
        #         sm = trimesh.creation.uv_sphere(radius=0.01)
        #         sm.visual.vertex_colors = [1.0,0.1,0.1]
        #         tfs = np.tile(np.eye(4), (len(kp_3d), 1, 1))
        #         pts = kp_3d.copy()
        #         pts = np.stack((pts[:, 0], -pts[:, 1], pts[:, 2]), axis=1)
        #         tfs[:, :3, :3] = Rotation.from_euler('x', 180, degrees=True).as_matrix()
        ...
        self.scene.remove_node(cam_node)
        # if smpl_kp_3d is not None:
        #     self.scene.remove_node(smpl_kp_mesh_node)
        # if kp_3d is not None:
        #     self.scene.remove_node(kp_mesh_node)
    ...
    def draw_smpl_kp_3d(self, smpl_kp_3d, img=np.zeros((224, 224, 3)), cam=np.array([1, 0, 0]),
                 focal_length=[5000, 5000], camera_rotation=np.eye(3), crop_info=None,
                 color_type=None, color=[0.0, 0.0, 1.0], scale_ratio=1., rgba_mode=False):
